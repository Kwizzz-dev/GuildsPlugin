...
public class BanPlayer
{
    public void ban(Player player, String[] args){
        ...
        if (guild.getBanList().contains(target.getUniqueId())){
            player.sendMessage(Messages.PLAYER_ALREADY_BANNED.toString());
            return;
        }
        ...
        guild.getBanList().add(target.getUniqueId());
        guild.save();
        Guilds.getProfiles().remove(target.getUniqueId());

        guild.broadcast(Messages.PLAYER_BANNED.toString().replaceAll("<player>", target.getName()));
        target.sendMessage(Messages.YOU_HAVE_BEEN_BANNED.toString());
    }
}
