...
public class MutePlayer
{
    public void mute(Player player, String[] args){
        ...
        if (guild.getMuteList().contains(target.getUniqueId())){
            player.sendMessage(Messages.PLAYER_ALREADY_MUTED.toString());
            return;
        }
        ...
        guild.broadcast(Messages.PLAYER_MUTED.toString().replaceAll("<player>", target.getName()));
        target.sendMessage(Messages.YOU_HAVE_BEEN_MUTED.toString());
    }
}
